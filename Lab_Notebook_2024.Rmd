...
pbs <- "PBS"
C1 <- "Heat Inactivated Serum"
C2 <- "Normal Human Serum"
R1 <- "0ug/mL RTX"
R2 <- "2.5ug/mL RTX"
Plate_Name <- "CDC Testing"
Plate_ID <- "CDC_24430_N2_Ramos"
...
Results: DNW  
- No clear differences between +HIAS/+RTX and +S/+RTX  
...
C1 <- "Heat Inactivated Serum"
C2 <- "Normal Human Serum"
EC50_ID <- "EC50_RTX_RAMOS_240430"

dil_ID = matrix(c(
                   "CNTR", "RTX6","RTX5","RTX4","RTX3","RTX2","RTX1",
                   "CNTR", "RTX6","RTX5","RTX4","RTX3","RTX2","RTX1",
                   "CNTR", "RTX6","RTX5","RTX4","RTX3","RTX2","RTX1",
                  "CNTR", "RTX6","RTX5","RTX4","RTX3","RTX2","RTX1",
                  "CNTR", "RTX6","RTX5","RTX4","RTX3","RTX2","RTX1",
...
p <- p + coord_equal()
p <- p + 
  scale_x_continuous(breaks = 1:7, expand = expansion(mult = c(0.01, 0.01))) +
  scale_y_continuous(breaks = 1:6, labels = LETTERS[1:6], expand = expansion(mult = c(0.01, 0.01)), trans = reverse_trans())
p <- p + geom_text(data = df2, aes(x = col, y = row, label = paste0(value)), size = 2,)
p <- p +
  labs(title = "96 Well Plate Map", subtitle = EC50_ID, x = "Col", y = "Row") + 
  #theme_prism("waves") + 
...
- Plate seeding protocol:
  ...
  2. Made RTX dilutions and added to respective wells in 25$\mu$L  
   ...
   |Dilution ID| [RTX Well] | Media Volume | Source |  [RTX WS] |
  | :-------: | :------: |:-------: | :------: | :------: |
  | RTX 1 | 1.66 mg/mL | 1200$\mu$L | 800$\mu$L stock | 6.66 mg/mL |
  | RTX 2 | 500 $\mu$g/mL | 1 mL | 1 mL RTX 1 | 3.33 $\mu$g/mL |
  | RTX 3 | 250 $\mu$g/mL | 1 mL | 1 mL  RTX 2 | 1.66 $\mu$g/mL |
  | RTX 4 | 125 $\mu$g/mL | 1 mL | 1 mL  RTX 3 | 0.833 $\mu$g/mL |
  | RTX 5 | 62.5 $\mu$g/mL | 1 mL | 1 mL  RTX 4 | 0.416 $\mu$g/mL |
  | RTX 6 | 31.25 $\mu$g/mL | 1 mL | 1 mL  RTX 5 | 0.208 $\mu$g/mL |
  | RTX 7 | 15.6 $\mu$g/mL | 1 mL | 1 mL  RTX 6 | 0.104 $\mu$g/mL |
  | RTX 8 | 7.8 $\mu$g/mL | 1 mL | 1 mL  RTX 7 | 0.052 $\mu$g/mL |
  | RTX 9 | 3.9 $\mu$g/mL | 1 mL | 1 mL  RTX 8 | 0.026 $\mu$g/mL |
  | CNTR | 0 $\mu$g/mL | 500 mL | - | 0 $\mu$g/mL |
 
 3. Plate is incubated for 48 hrs at 37C
...
- Results: clear difference between NHS and HIAS
...
R2 <- "2.5ug/mL RTX"
Plate_Name <- "CDC Testing"
Plate_ID <- "CDC_240624_RAMOS"

dil_ID = matrix(c(R1,R1,R1,
                  R1,R1,R1
...
p <- p + 
  scale_x_continuous(breaks = 1:3, , 
                     labels = c("1", "2", "3"), 
                     expand = expansion(mult = c(0.01, 0.01))) +
  scale_y_continuous(breaks = 1:2, labels = c("10% Serum", "25% Serum"), expand = expansion(mult = c(0.01, 0.01)), trans = reverse_trans())
p <- p + geom_text(data = df2, aes(x = col, y = row, label = paste0(value)), size = 2,)
p <- p +
  labs(title = Plate_ID, x = "Col", y = "Row") + 
  #theme_prism("waves") + 
...
## RAMOS - RTX In Vitro CDC Pressure
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Plate 1 data
plate1_labels <- c("C1 -RTX", "C2 -RTX", "C3 -RTX", "C4 -RTX", "C5 -RTX", "C6 -RTX")
plate1_df <- data.frame(
  row = rep(1:2, each = 3),
  col = rep(1:3, times = 2),
  label = plate1_labels
)
...
# Plot Plate 1
p1 <- ggplot(data = plate1_df) + 
  geom_circle(aes(x0 = col, y0 = row, r = 0.45), fill = "lightblue", color = "black") +
  coord_equal() +
  scale_x_continuous(breaks = 1:3, labels = c("1", "2", "3"), expand = expansion(mult = c(0.01, 0.01))) +
  scale_y_continuous(breaks = 1:2, labels = c("25% Serum", "25% Serum"), expand = expansion(mult = c(0.01, 0.01)), trans = reverse_trans()) +
  geom_text(aes(x = col, y = row, label = label), size = 4) +
  labs(title = "RAMOS BC 1 RTX DP - Dose 1 Plate 1", x = " ", y = " ") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
print(p1)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Plate 2 data
plate2_labels <- c("R1 +RTX", "R2 +RTX", "R3 +RTX", "R4 +RTX", "R5 +RTX", "R6 +RTX")
plate2_df <- data.frame(
  row = rep(1:2, each = 3),
  col = rep(1:3, times = 2),
  label = plate2_labels
)
...
# Plot Plate 2
p2 <- ggplot(data = plate2_df) + 
  geom_circle(aes(x0 = col, y0 = row, r = 0.45), fill = "purple", color = "black") +
  coord_equal() +
  scale_x_continuous(breaks = 1:3, labels = c("1", "2", "3"), expand = expansion(mult = c(0.01, 0.01))) +
  scale_y_continuous(breaks = 1:2, labels = c("25% Serum", "25% Serum"), expand = expansion(mult = c(0.01, 0.01)), trans = reverse_trans()) +
  geom_text(aes(x = col, y = row, label = label), size = 4) +
  labs(title = "RAMOS BC 1 RTX DP - Dose 1 Plate 2", x = "Col", y = "Row") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
print(p2)
